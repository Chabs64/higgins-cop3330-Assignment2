...
public class GuessNumberGame {
    ...
    public static void main(String[] args) {
        ...
        while(true)
        {
            ...
            int Level = Integer.parseInt(GetNumber());
            System.out.println("I have my number. What's your guess?");
            int rightAnswer = GenerateNumber(Level);
            int NumGuess = 0;
            while(true)
            {
                NumGuess++;
                if (takeAGuess(rightAnswer))
                {
                    break;
                }
            }
    ...
    public static boolean takeAGuess(int Answer)
    {
        int Guess = Integer.parseInt(GetNumber());

        switch (HighOrLow(Guess, Answer))
        {
            case 0:
                return true;
            case 1:
    ...
    public static int HighOrLow(int guess, int answer)
    {
        if (guess == answer)
        {
            return 0;
